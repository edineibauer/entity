...
class Entitye
{
    ...
    private function checkObjectEntity($objeto, string $field, bool $recursivo = false)
    {
        ...
        } elseif (is_object($objeto) && is_a($objeto, "Entity\Entitye")) {
    ...
    private function checkDataforFk($value, string $title, array $metadados)
    {
        if (is_null($value)) {
            ...
            $obj = new Entitye($metadados['table']);
            $obj->setData($value);
            if ($obj->getErro()) {
        ...
        } elseif (!is_object($value) || !is_a($value, "Entity\Entitye") || $metadados['table'] !== $value->getEntity()) {
     ...
     * @return Entitye
     */
    private function getEntityFromId(int $id, string $table): Entitye
    {
        $objData = new EntityData($table);
